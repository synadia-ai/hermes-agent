...
- `docs/nats-gateway-progress.md` is the phase-by-phase progress log (completed through Phase 11). The decision log at the bottom captures non-obvious moment-of-landing context that the design doc alone doesn't cover.
- `docs/nats-gateway.md` points to the user-facing setup guide at `website/docs/user-guide/messaging/nats.md`.
- Agent-side SDKs (split as of client v0.5 / agent v0.1, 2026-04-30; on PyPI as of 2026-05-01; current floors `synadia-ai-agents>=0.7` / `synadia-ai-agent-service>=0.3` since 2026-05-04): two distributions in the `synadia-ai/synadia-agents` monorepo. Client SDK [`synadia-ai-agents`](https://pypi.org/project/synadia-ai-agents/) (`synadia_ai.agents`, wire types: `Envelope`, `Attachment`, chunk classes, errors incl. `StreamStalledError` / `StreamMaxWaitExceededError` / `AgentsClosedError` (new in 0.6), discovery, `load_context_options`) — source at `../synadia-agents/client-sdk/python`; agent SDK [`synadia-ai-agent-service`](https://pypi.org/project/synadia-ai-agent-service/) (`synadia_ai.agent_service`, host surface: `AgentService`, `PromptStream`, `PromptHandler`) — source at `../synadia-agents/agent-sdk/python`. `AgentService` (renamed from `Agent` in v0.3) wraps micro-service registration, heartbeats, the `agents.status` request endpoint, chunk wrapping, terminators, error headers, `max_payload` clamp-down at `start()`, and mid-stream `stream.ask()` automatically. Wire-shape history: 0.6/0.2 added an `Agent.prompt(max_wait_s=...)` ceiling + per-NC mux reply-inbox (purely additive); 0.7/0.3 restored `metadata.session` (§3.2) + `HeartbeatPayload.session` (§8.3) + status-reply `session` (§8.7) which were mistakenly dropped during the 2026-04-28 session-name collapse — the SDK handles all three internally, so no adapter-side code change. Protocol stays at `"0.3"`. Protocol spec: `../nats-agent-sdk-docs/core-protocol.md` (still a separate repo, v0.3). Both SDKs are pulled in transparently by the `hermes-agent[nats]` extra; `uv sync --all-extras --locked` resolves them from PyPI. The previous `[tool.uv.sources]` editable-sibling override has been removed (2026-05-01).

The NATS adapter is the canonical example in the codebase of an **adapter-owned `AIAgent`** (api_server-style, not `handle_message`-routed) + **`asyncio.Lock` serialization** + **`request_interaction` approval hook**. v0.3 collapses session into the 5th subject token (`session_name`), so each `AgentService` serves exactly one session — multi-session deployments use Hermes profile isolation (one profile = one service = one `session_name`). If you're building another transport with the same shape (programmatic request/reply, non-edit-based streaming), copy patterns from `nats.py`, not `telegram.py`.
