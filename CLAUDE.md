...
- Agent-side SDKs (split as of client v0.5 / agent v0.1, 2026-04-30): two distributions in the `synadia-ai/synadia-agents` monorepo. Client SDK `synadia-ai-agents` (`synadia_ai.agents`, wire types: `Envelope`, `Attachment`, chunk classes, errors, discovery, `load_context_options`) at `../synadia-agents/client-sdk/python`; agent SDK `synadia-ai-agent-service` (`synadia_ai.agent_service`, host surface: `AgentService`, `PromptStream`, `PromptHandler`) at `../synadia-agents/agent-sdk/python`. `AgentService` (renamed from `Agent` in v0.3) wraps micro-service registration, heartbeats, the `agents.status` request endpoint, chunk wrapping, terminators, error headers, `max_payload` clamp-down at `start()`, and mid-stream `stream.ask()` automatically. Protocol spec: `../nats-agent-sdk-docs/core-protocol.md` (still a separate repo, v0.3). Both SDKs are resolved locally via `[tool.uv.sources]` in `pyproject.toml` until they ship on PyPI; `uv sync --all-extras --locked` resolves them transparently — no manual `uv pip install -e ...` step needed.
